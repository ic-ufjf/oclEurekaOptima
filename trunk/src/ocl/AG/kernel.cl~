...
/*
 KERNEL 4:
 Utiliza TAMANHO_POPULACAO itens de trabalho, cada 1 responsável por selecionar 2 pais
 e gerar 1 filho, realizando crossover e mutação.
*/
__kernel void iteracao_1_por_work_item(__global individuo *pop, 
			const int geracao, 
			int fixed_seed, 
			__global individuo *newPop,
			__global struct r123array4x32 *counter)
{    
    ...
    */
    ...
    recombinacao4(pais, filhos, TAXA_DE_RECOMBINACAO, &rng);	
...
void recombinacaoParalela(int lid, __local individuo *pais, 
			  __local individuo *filhos, 
			  __local float* aleatorio, 
			  __local int* pontoCrossOver, 
			  cburng4x32 *rng, float chance){
	
	
	int itens = TAMANHO_INDIVIDUO / get_local_size(0);
	
	int start = lid*itens, end = min(lid*itens+itens, TAMANHO_INDIVIDUO);
	
	if(*aleatorio<chance){	
		
		for(int i=start; i<end;i++){
			
   		    short parte = i < *pontoCrossOver ? 0 : 1;		
			
		    filhos[0].genotipo[i] = pais[parte].genotipo[i];
		    filhos[1].genotipo[i] = pais[1-parte].genotipo[i];		
		}	
	}
	else{	
...
void mutacaoParalela(int lid, __local individuo * filhos,  cburng4x32 *rng){

	short itens = TAMANHO_INDIVIDUO / get_local_size(0);
	...
	for(int i=start; i<end;i++){
	    ...
	    if(u_rand(rng) < (float)TAXA_DE_MUTACAO){	
	        ...
	        filhos[0].genotipo[i] = filhos[0].genotipo[i] == 1 ? 0 : 1;
	    }
	    
	    if(u_rand(rng) < (float)TAXA_DE_MUTACAO){		
	       
	        filhos[1].genotipo[i] = filhos[1].genotipo[i] == 1 ? 0 : 1;
	    }
	}	
}
...
*/
__kernel void iteracao_2_por_work_group2(__global individuo *pop, 
			const int geracao, 
			int fixed_seed, 
			__global individuo *newPop,
			__global struct r123array4x32 *counter)
{    
    ...
    */
    ...
    __local float al;
     ...
     if(lid == 0){
    	...
    	pais[0] = pop[torneio(pop, gid, &rng)];
    	pais[1] = pop[torneio(pop, gid+1, &rng)];

	al = u_rand(&rng);
      
        pontoCrossOver = rand(&rng) % (TAMANHO_INDIVIDUO);

	recombinar = al < (float)TAXA_DE_RECOMBINACAO;	
    }        
    ...
    */
    ...
    recombinacaoParalela(lid, pais, filhos, &al, &pontoCrossOver, &rng, TAXA_DE_RECOMBINACAO);

    /*
	Mutação
    */
    
    mutacaoParalela(lid, filhos, &rng);
