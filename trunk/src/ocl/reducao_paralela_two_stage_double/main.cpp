...
int get_max_compute_units(cl_device_id device){
  ...
  //if(buf_uint % 2 != 0) return buf_uint+1;
...
void soma_paralela(double *x, const int elementos){
    ...
    int compute_units  = get_max_compute_units(device); //get_max_compute_units(device);
    ...
    clSetKernelArg(k,1, sizeof(float)*compute_units*100, NULL);
...
double soma_sequencial(double *x, int elementos){

    double aux=0; int i;

    for(i=0;i<elementos;i++) aux+=(double)x[i];
...
int main(int argc, char * argv[])
{
    ...
    double valor = soma_sequencial(x, elementos);
