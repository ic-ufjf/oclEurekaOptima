...
void obtem_fenotipo_individuo(individuo *p){
    ...
    for(i=0; i<DIMENSOES_PROBLEMA; i++, j+=TAMANHO_VALOR){
       ...
       //Calcula o fenótipo, considerando o deslocamento necessário para o espaço de busca
       p->fenotipo[i] = binario_para_decimal(p, j, j+TAMANHO_VALOR) - 127;
...
int funcao_de_avaliacao(individuo *p){
    ...
    for(i=0;i < DIMENSOES_PROBLEMA; i++){
        ...
        for(j=0;j<=i;j++){
            somatorio2+= p->fenotipo[i];
        }

        somatorio += pow(somatorio2,2);
    }
