...
int torneio2(__local float *aptidao, int indice_participante, cburng4x32 *rng) {	
    ...
    while(i < TAMANHO_TORNEIO) {
        ...
        if(aptidao[aleatorio] > aptidao[vencedor]){
            vencedor = aleatorio;
        }
...
void crossover_um_ponto(individuo *pais, individuo *filhos, cburng4x32 *rng){
    ...
    int ponto = rand(rng) % (TAMANHO_INDIVIDUO/8);
    ...
    for(i=ponto;i<TAMANHO_INDIVIDUO/8;i++){
...
void recombinacao(__private individuo *pais, __private  individuo *filhos, float chance, cburng4x32 *rng){
...
void mutacao(individuo *p, float chance, cburng4x32 *rng){
    ...
    for(int i=0;i<mutacoes;i++) {
        ...
        p->genotipo[aleatorio] = !p->genotipo[aleatorio];
...
{    
   ...
   __private int i;
    __private individuo pais[2];
    __private individuo filhos[2]; 	
  

    int indice_pais[2];
    
    __local float lpop[TAMANHO_POPULACAO];
    
    for(uint iter = 0; iter < ceil( TAMANHO_POPULACAO / (float) lsize ); ++iter )
    {
        if( iter * lsize + lid < TAMANHO_POPULACAO )
        {
            lpop[iter * lsize + lid] = pop[iter * lsize + lid].aptidao;	
	    }		
    }
    
    barrier(CLK_LOCAL_MEM_FENCE);        

    indice_pais[0] = torneio2(lpop, tid, &rng);
    indice_pais[1] = torneio2(lpop, tid+1, &rng);
    
    for(i = 0; i < TAMANHO_INDIVIDUO/8; i++){
        pais[0].genotipo[i] = pop[indice_pais[0]].genotipo[i];
        pais[1].genotipo[i] = pop[indice_pais[1]].genotipo[i];
    } 
    ...
    */
	...
	int mutacoes = ceil(TAMANHO_INDIVIDUO * TAXA_DE_MUTACAO);
    int aleatorio=0;
    
    for(i=0;i<mutacoes;i++) {
	
        aleatorio = rand(&rng) % TAMANHO_INDIVIDUO;
        filhos[0].genotipo[aleatorio] = !filhos[0].genotipo[aleatorio];
    ...
    for(i=0;i<mutacoes;i++) {
        ...
        aleatorio = rand(&rng) % TAMANHO_INDIVIDUO;
        filhos[1].genotipo[aleatorio] = !filhos[1].genotipo[aleatorio];
    }	
    ...
    for(i = 0; i < TAMANHO_INDIVIDUO/8; i++){
        newPop[tid].genotipo[i]   = 1;//filhos[0].genotipo[i];        
        newPop[tid+1].genotipo[i] = 1;//filhos[1].genotipo[i];
    }
