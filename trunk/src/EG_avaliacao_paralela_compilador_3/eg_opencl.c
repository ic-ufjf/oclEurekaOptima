...
void compila_programa(t_prog * pop){
    ...
    std::string fitness_string = ToString("void avaliaprograma(int p, int tid, int lid, int local_size, \n")+

    ToString("#ifdef Y_DOES_NOT_FIT_IN_CONSTANT_BUFFER\n ")+
	ToString("__global const \n ")+
    ToString("#else \n ")+
    ToString("__constant  \n ")+
    ToString("#endif  \n ")+
    ToString("float * dataBase,     \n ")+
