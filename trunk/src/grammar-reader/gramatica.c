...
int Decodifica(t_regra * Gramatica, int * fenotipo, t_item_programa * programa){
	...
	int counter;
	for(counter=0;counter < program_ctr; counter++ ){
		GetNomeElemento(&programa[counter].t, nomeElemento);
		printf(" (%s, %d)  ",  nomeElemento, programa[counter].proximo);
	}
	...
	int k;
	for(k=0;k < program_ctr; k++ ){

		GetNomeElemento(&programa[k].t, nomeElemento);
		printf(" (%s) ", nomeElemento);
	}

	printf("\n\nIndividuo na ordem certa:\n");
