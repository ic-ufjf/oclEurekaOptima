...
void soma_paralela(float *x, const int elementos){
    ...
    cl_mem mem1;
    mem1=clCreateBuffer(context, CL_MEM_READ_WRITE, elementos * sizeof(float), NULL, &error);

    error=clEnqueueWriteBuffer(cq, mem1, CL_TRUE, 0,  elementos * sizeof(float), x, 0, NULL, &event1);
    ...
    clSetKernelArg(k,1, sizeof(float)*compute_units*20, NULL);
    clSetKernelArg(k,2, sizeof(elementos), &elementos);
    clSetKernelArg(k,3, sizeof(mem1), &mem1);

    int t = (int)pow( (float)2.0, (int) ceil( log2( (float) elementos ) ) );

    worksize = (size_t) std::min(compute_units, t);

    error=clEnqueueNDRangeKernel(cq, k, 1, NULL, &worksize, &worksize, 0, NULL, &event2);

    error=clEnqueueReadBuffer(cq, mem1, CL_TRUE, 0,  elementos * sizeof(float), x, 0, NULL, &event3);
...
float soma_sequencial(float *x, int elementos){

    float aux=0; int i;

    for(i=0;i<elementos;i++) aux+=(float)x[i];
...
int main(int argc, char * argv[])
{
    ...
    for(int i=0;i<elementos; i++) x[i] = (float)1;

    float valor = soma_sequencial(x, elementos);
